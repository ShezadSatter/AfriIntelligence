...
router.get('/status', async (req, res) => {
  try {
    const stats = {
      totalContent: await dbModels.Content.countDocuments(),
      bySubject: {},
      byGrade: {}
    };
    ...
    for (const subject of subjects) {
      const count = await dbModels.Content.countDocuments({ subject: subject._id });
    ...
    for (const grade of grades) {
      const count = await dbModels.Content.countDocuments({ grade: grade._id });
      stats.byGrade[`Grade ${grade.level}`] = count;
    }
